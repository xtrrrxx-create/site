...
function renderFilteredProducts() {
    ...
    const renderCard = p => {
        ...
        // NOTE: the `img` column historically holds the QC photo. `ref_img` is
        // the (optional) clean reference/model photo added later in admin.
        const safeQc = (isHttp && !isKnownPlaceholder) ? rawImg : '';   // p.img = QC
        const rawRef = String(p.ref_img || '').trim();
        const safeRef = /^https?:\/\//i.test(rawRef) ? rawRef : '';     // model photo

        // Card image rules (picks.ly style):
        //  - both reference + QC → reference is the base, QC fades in on hover
        //  - only QC (no reference) → QC IS the base photo (no hover swap)
        //  - only reference → reference
        const baseImg = safeRef || safeQc;          // reference wins; else QC
        const hasHover = !!(safeRef && safeQc);     // swap only when both exist
        ...
        // QC overlay only when there's a reference photo to swap FROM. Lazy; sits
        // on top of the base image and crossfades in on hover.
    ...
    // Group the visible products by brand (derived from the title) so the grid
...
function getPages() {
        ...
        <div style="position:relative;">
            ...
            <div class="pl-hero">
                ...
                <div class="pl-cats">
                    ...
                    ${CATEGORIES.map(cat => `<button class="pl-cat ${(cat !== 'All' && filterState.category === cat) ? 'active' : ''}" data-home-cat="${cat}"><span>${cat}</span></button>`).join('')}
                </div>
            </div>
    ...
    `,
        ...
        <style>
            ...
            .qc-actions { display: flex; flex-direction: column; align-items: center; gap: 0.5rem; margin-top: 0.75rem; }
            .qc-actions .card-btn-buy, .qc-actions .card-btn-qc { flex: none; width: 280px; max-width: 100%; height: 36px; font-size: 14px; font-weight: 500; }
...
function renderFavoritesContainer() {
    ...
    const heartSvg = `<svg viewBox="0 0 24 24" width="44" height="44" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 1 0-7.78 7.78L12 21.23l8.84-8.84a5.5 5.5 0 0 0 0-7.78z"/></svg>`;

    // Favourites are account-bound — prompt login when signed out.
    if (window.jfAuth && !window.jfAuth.isLoggedIn()) {
        c.innerHTML = `<div class="fav-empty" style="grid-column:1/-1;">
            ${heartSvg}
            <p>Log in with Discord to save and sync your favorites.</p>
            <button class="set-discord-btn" id="fav-login-btn" type="button" style="width:auto;padding:0.6rem 1.2rem;">
        ...
        c.querySelector('#fav-login-btn')?.addEventListener('click', () => window.jfAuth.signIn());
        return;
    }
